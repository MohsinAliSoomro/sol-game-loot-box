...
import { useProjectDashboardStats, useProjectRecentActivity } from "@/lib/hooks/useProjectDashboardStats";
import { useProject } from "@/lib/project-context";

export default function ProjectAdminDashboard() {
  const { getProjectTokenSymbol, projectToken } = useProject();
  const projectTokenSymbol = getProjectTokenSymbol();
  const stats = useProjectDashboardStats();
  const { activities, loading: activitiesLoading, error: activitiesError } = useProjectRecentActivity();
  ...
  return (
    <div className="p-6">
      ...
      {/* Vault Address Finder - TODO: Copy component from spinloot_dashboard */}
      {showVaultFinder && (
        <div className="mb-8 bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <p className="text-gray-600">Vault Address Finder - Coming soon</p>
        </div>
      )}

      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 mb-8">
        {/* Total Project Token Spent Card */}
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">Total {projectTokenSymbol} Spent</p>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-500 text-sm mt-1">{projectToken?.exchange_rate_to_sol ? `1 ${projectTokenSymbol} = ${(1 / (projectToken.exchange_rate_to_sol || 1000)).toFixed(6)} SOL` : `1000 ${projectTokenSymbol} = 1 SOL`}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">Total {projectTokenSymbol} Withdrawn</p>
