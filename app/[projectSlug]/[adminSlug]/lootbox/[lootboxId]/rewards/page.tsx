...
import { useProjectRewards } from '@/lib/hooks/useProjectRewards';
...
export default function LootboxRewards() {
  ...
  const lootboxId = params?.lootboxId as string;
  
  const [lootboxName, setLootboxName] = useState(`Lootbox #${lootboxId}`);
  
  // Fetch rewards for this specific lootbox (product_id = lootboxId)
  const { rewards, loading, error, addReward, updateReward, deleteReward } = useProjectRewards(lootboxId);
  ...
  const { getUsedNFTMints } = useProjectJackpotPools();
  ...
  useEffect(() => {
    const fetchLootboxName = async () => {
      try {
        ...
        if (!error && data) {
          setLootboxName(data.name);
        }
      } catch (err) {
        console.error('Error fetching lootbox name:', err);
      }
    };
  ...
  const resetAndShowModal = (reward: any = null) => {
    if (reward) {
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <div className="flex items-center space-x-3">
            <button 
              onClick={() => router.push(`/${projectSlug}/${adminSlug}/settings`)} 
          ...
          <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="flex space-x-4 border-b pb-4">
                <button 
                  type="button" 
                  onClick={() => setSelectedRewardType('item')} 
                ...
                <button 
                  type="button" 
                  onClick={() => setSelectedRewardType('nft')} 
                  className={`flex-1 py-2 px-4 rounded-lg border ${
                ...
                <button 
                  type="button" 
                  onClick={() => setSelectedRewardType('sol')} 
                  className={`flex-1 py-2 px-4 rounded-lg border ${
                  ...
                  {adminWalletLoading ? (
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-600 bg-blue-50 p-3 rounded-lg mt-4">
                        <p className="font-medium mb-1">ℹ️ SOL Reward Info:</p>
                        <p>• SOL will be transferred from your wallet to the vault</p>
                        <p>• Users can win and withdraw SOL rewards</p>
                        <p>• Grayed out amounts exceed your current balance</p>
                      </div>
                    </div>
                  )}
                </>
              ...
              {(selectedRewardType === 'item' || selectedRewardType === 'token') && (
                <div>
