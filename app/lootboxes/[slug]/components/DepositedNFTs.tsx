 ...
 */
export default function DepositedNFTs({ productId }: DepositedNFTsProps) {
  ...
  useEffect(() => {
    const loadDepositedNFTs = async () => {
      try {
        ...
        // Load NFT reward percentages from database for this specific lootbox
        ...
        if (productId) {
          nftQuery = nftQuery.eq('product_id', productId);
        }
        
        const { data: nftRewards, error: nftRewardError } = await nftQuery;
        
        if (nftRewardError) {
          console.warn('⚠️ Could not load NFT reward percentages from database:', nftRewardError);
        }
        
        console.log("📍 NFT rewards from database:", nftRewards);
        
        // Extract mint addresses from database
        const mintAddresses = (nftRewards || []).map((r: any) => r.mint_address).filter(Boolean);
        ...
        // Use database reward data as base and enhance with metadata
        const nftsWithPercentages = (nftRewards || []).map((dbReward: any) => {
          const metadataMatch = nftMetadata.find(nft => nft.mint === dbReward.mint_address);
          ...
          return {
            ...
            percentage: dbReward.percentage || 0
          };
        });
        ...
        // Load SOL rewards from database for this specific lootbox
        ...
        // Filter by product_id if provided
        if (productId) {
          solQuery = solQuery.eq('product_id', productId);
        }
        
        const { data: solRewards, error: solError } = await solQuery;
        ...
        if (solError) {
          ...
          const mappedSolRewards: SOLReward[] = (solRewards || []).map((r: any, idx: number) => ({
            ...
            image: r.reward_image ? getImageUrl(r.reward_image) : 'https://raw.githubusercontent.com/solana-labs/token-list/main/assets/mainnet/So11111111111111111111111111111111111111112/logo.png',
        ...
        // Load ITEM rewards from database for this specific lootbox
        // Items are rewards that are NOT SOL and don't have both collection AND token_id (not NFTs)
        // An item is: has reward_name, NOT SOL, and (collection is null OR token_id is null)
        ...
        // Filter by product_id if provided
        if (productId) {
          itemQuery = itemQuery.eq('product_id', productId);
        }
        
        const { data: allTokenRewards, error: tokenError } = await itemQuery;
        
        console.log("🔍 All non-SOL token rewards:", allTokenRewards);
        
        // Filter items: exclude NFTs (which have both collection and token_id)
        // Items are rewards that don't have both collection AND token_id set
        const itemRewards = (allTokenRewards || []).filter((r: any) => {
          ...
          const isItem = !(hasCollection && hasTokenId);
          console.log(`  - ${r.reward_name}: collection=${hasCollection}, token_id=${hasTokenId}, isItem=${isItem}`);
          return isItem;
        });
        
        console.log("✅ Filtered ITEM rewards:", itemRewards);
        
        const itemError = tokenError;

        if (itemError) {
          console.warn('⚠️ Could not load ITEM rewards from database:', itemError);
          setItemRewards([]);
        } else {
          const mappedItemRewards: ItemReward[] = itemRewards.map((r: any, idx: number) => ({
            id: r.id || (4000 + idx),
            name: r.reward_name || 'Item',
            image: getImageUrl(r.reward_image),
            value: r.reward_price || '',
            percentage: r.percentage || 0,
            color: `hsl(${(idx * 55 + 120) % 360}, 70%, 60%)`,
          }));
          
          console.log("✅ Loaded ITEM rewards from database:", mappedItemRewards);
          setItemRewards(mappedItemRewards);
        }
  ...
  return (
    <div className="w-full px-4">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-5">
        ...
        {itemRewards.map((itemReward, index) => (
          <div
            key={itemReward.id}
            className="h-48 bg-white border border-orange-300 rounded-lg shadow-md text-orange-800 flex flex-col items-center
                     transition-all duration-300 hover:shadow-lg group overflow-hidden"
          ...
          >
            ...
            <div className="box-header flex justify-between w-full items-center p-2">
              <div className="font-bold text-center text-sm text-orange-800 flex justify-center items-center">
                <svg width="30" height="30" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 256 256" xmlSpace="preserve">
                  ...
                  <path d="m191.12 49.473-26.822-5.428-13.488-2.729-24.703-4.999a15.668 15.668 0 0 0-6.215 0L54.88 49.473c-7.928 1.604-13.332 8.981-12.471 17.024l6.165 57.586a110.403 110.403 0 0 0 58.333 85.933 34.541 34.541 0 0 0 32.186 0 110.4 110.4 0 0 0 58.333-85.933l6.165-57.586c.861-8.043-4.542-15.419-12.471-17.024z" style={{fill:"#ed6e7a"}}/>
                  <path className="st0" d="m191.417 48.003-26.822-5.428a1.5 1.5 0 1 0-.595 2.941l26.822 5.428a14.104 14.104 0 0 1 11.276 15.394l-6.164 57.586c-3.869 36.141-25.38 67.829-57.541 84.766a33.032 33.032 0 0 1-30.789 0c-32.161-16.936-53.671-48.624-57.541-84.765L43.9 66.337a14.106 14.106 0 0 1 11.277-15.394l65.012-13.157a14.11 14.11 0 0 1 5.621 0l24.703 5a1.5 1.5 0 1 0 .595-2.941l-24.703-5a17.085 17.085 0 0 0-6.81 0L54.583 48.003a17.095 17.095 0 0 0-13.666 18.654l6.164 57.586c3.976 37.137 26.079 69.698 59.127 87.101A35.998 35.998 0 0 0 123 215.496a36.01 36.01 0 0 0 16.792-4.153c33.047-17.402 55.15-49.963 59.127-87.1l6.164-57.586a17.092 17.092 0 0 0-13.666-18.654z"/>
                  <text x="118" y="130" textAnchor="middle" fontSize="50" fill="white" fontWeight="bold" className="text-orange-800">
                    ITEM
                  </text>
                </svg>
              </div>
            ...
            <div className="w-24 md:h-24 h-16 group-hover:scale-105 transition-transform duration-300 ml-0">
              <Image
                src={itemReward.image}
                alt={itemReward.name}
                width={250}
                height={150}
                className="object-contain drop-shadow-md rounded-lg"
                onError={(e) => {
                  // Fallback to default image if item image fails to load
            ...
            <div className="mt-2 reward font-bold text-center relative bottom-7 w-full py-2 text-xs rounded-lg  flex justify-center items-center gap-2 md:-top-1 top-11">
              <span className="truncate max-w-full px-2">
                {itemReward.name} {itemReward.value ? `(${itemReward.value})` : ''}
              </span>
            </div>
          </div>
        ))}
