-- Create Jackpot System Database
...
CREATE TABLE jackpot_pools (
    ...
    ticket_price DECIMAL(10,2) NOT NULL DEFAULT 1,
...
-- Create jackpot_tickets table for purchases
...
-- Create policies
CREATE POLICY "Anyone can view active jackpots" ON jackpot_pools FOR SELECT USING (is_active = true);
CREATE POLICY "Anyone can view all jackpots" ON jackpot_pools FOR SELECT USING (true);
CREATE POLICY "Anyone can view tickets" ON jackpot_tickets FOR SELECT USING (true);
CREATE POLICY "Anyone can insert tickets" ON jackpot_tickets FOR INSERT WITH CHECK (true);
...
SELECT * FROM jackpot_pools ORDER BY id;
